...
all_source={di.get("source") for di in leads}#set comprehension
print(all_source)
...
qualified=[di for di in leads if di.get("status")=="Qualified"]
print(len(qualified))
# qualified_count={q:all_source_list.count(q) for q in all_source_list}
...
"""
qualifid_rate=total_no_qualified_lead/total_no_leads
...
# conversion_rate
"""
conversion_rate=total__no_of_conerted_leads/total_no_of_lead

"""
conversion_rate= status_count.get("Converted")/sum(status_count.values())
print(conversion_rate)

unqualified_rate= status_count.get("Unqualified")/sum(status_count.values())
print(unqualified_rate)


# conversion rate of google ads

# google_ads_converted_rate= total_no_of coverted_google_ads/total_no_of_google_ads
google_Ad_status=[di.get("status") for di in leads if di.get("source")=="Google Ads"]
google_Ad_status_summary={s:google_Ad_status.count(s) for s in google_Ad_status}
print(google_Ad_status_summary)

google_Ad_conversion_rate=google_Ad_status_summary.get("Converted",0)/sum(google_Ad_status_summary.values())
print("google Ad conversion rate=",google_Ad_conversion_rate)
